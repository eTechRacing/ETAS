...
/*
 * Detector defines MATLAB_TARGET_API_VERSION based on any combination
 * (including none) of input macros of two kinds: legacy .vs. release-based,
 * and hard .vs. soft.
 *
 * Legacy input macros (can be undefined or defined):
 *     MEX_DOUBLE_HANDLE
 *     MX_COMPAT_32
 *
 * Release-based hard input macros (can be undefined or defined to a legal release tag):
 *     MATLAB_MEXCMD_RELEASE
 *     MATLAB_MEXSRC_RELEASE
 *
 * Release-based soft input macro
 *     MATLAB_DEFAULT_RELEASE
 *
 * Output macros:
 *     MATLAB_TARGET_API_VERSION (must be defined)
 *     2 legacy macros (could be defined or undefined)
 *
 * These are the generic rules:
 * - Legacy macros are hard and serve as input and output macros.
 * - Hard release-based macros cannot be used with legacy macros.
 * - Soft macro is used only if there are no hard macros.
 *
 * Detector generates a preprocessor error if both legacy and hard released-based
 * macros are defined.
 *
 * Detector defines release tokens to corresponding API version. In addition to
 * the release macros Detector defines the following auxiliary macros:
 *     MW_REL2VER	     – Release-to-version macro
 *     MW_FIRST_API_VERSION  – First known serial version
 *     MW_LATEST_API_VERSION - Version of the current release
 *
 * The first macro translates from a release token to a corresponding version.
 * The latter two macros allow to generate a preprocessor error if a translated
 * version is not between the first and the last versions.
 *
 * If none of the input macros are defined Detector sets MATLAB_TARGET_API_VERSION to
 * MW_LATEST_API_VERSION or MW_FIRST_API_VERSION for SC ALTBUILD.
 *
 * If only the soft macro is defined Detector sets MATLAB_TARGET_API_VERSION to the
 * version that corresponds to the proposed release token.
 *
 * Hard input macros override the soft macro.
 */
...
/* None of the input macros are defined. Use LATEST unless in ALTBUILD */

#if defined(MW_ALTBUILD_ENABLED) && !defined(MX_INTERLEAVED_COMPLEX)
#define MATLAB_TARGET_API_VERSION MW_FIRST_API_VERSION
#else
...
/* Temporary support for TARGET_API_VERSION */
