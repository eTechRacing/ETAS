...
for i = 1:length(SI);
    for j = 1:length(VS);
        ...
        for k=1:10     
            ...
            SvygammaFL(i,j)=FzFL(i,j)*(PVY3+PVY4*dfzFL(i,j))*gammaFL(i,j)*lambdaKygamma*lambdauy*z2;
            ...
            KyalphaFL(i,j)=PKY1*Fz0*(1+PPY1*dpiFL)*sin(PKY4*atan(FzFL(i,j)/((PKY2+PKY5*gammaFL(i,j)^2)*Fz0*(1+PPY2*dpiFL))))*(1-PKY3*abs(gammaFL(i,j)))*lambdaKyalpha*z3;
            ...
            uyFL(i,j)=(PDY1+PDY2*dfzFL(i,j))*(1+PPY3*dpiFL+PPY4*dpiFL^2)*(1-PDY3*gammaFL(i,j)^2)*lambdauy;
            DyFL(i,j)=uyFL(i,j)*FzFL(i,j)*z2;
            EyFL(i,j)=(PEY1+PEY2*dfzFL(i,j))*(1+PEY5*gammaFL(i,j)^2-(PEY3+PEY4*gammaFL(i,j))*sign(alphayFL(i,j)))*lambdaEy;
