...
void qrpf_X6HM2OpU(real_T A[4], int32_T ia0, int32_T m, int32_T n, real_T tau[2],
                   int32_T jpvt[2])
{
  real_T vn1[2];
  real_T vn2[2];
  ...
  vn2[1] = 0.0;
  for (b_j = 0; b_j < n; b_j++) {
    vn1[b_j] = xnrm2_5D3RImKI(m, A, (b_j << 1) + ia0);
    vn2[b_j] = vn1[b_j];
  }
  ...
  for (b_j = 0; b_j < minmn; b_j++) {
    ...
    if (b_j + 1 < m) {
      ...
      tau[b_j] = 0.0;
      if (mmi > 0) {
        smax = xnrm2_5D3RImKI(mmi - 1, A, ii + 2);
        if (smax != 0.0) {
          ...
          if (fabs(smax) < 1.0020841800044864E-292) {
            ...
            smax = rt_hypotd_snf(s, xnrm2_5D3RImKI(mmi - 1, A, ii + 2));
            if (s >= 0.0) {
              smax = -smax;
            }
    ...
    if (b_j + 1 < n) {
      ...
      if (pvt + 1 > 0) {
        ...
        if (nmi + 1 != 0) {
          ...
          ix = 0;
          iy = ((nmi << 1) + ii) + 3;
          for (ii_tmp = ii + 3; ii_tmp <= iy; ii_tmp += 2) {
            c_ix = ii;
            s = 0.0;
            e = ii_tmp + pvt;
    ...
    while (c_j + 1 <= n) {
      ...
      if (vn1[c_j] != 0.0) {
        ...
        if (s <= 1.4901161193847656E-8) {
          if (b_j + 1 < m) {
            vn1[c_j] = xnrm2_5D3RImKI(mmi - 1, A, ii + 1);
            vn2[c_j] = vn1[c_j];
          } else {
            vn1[c_j] = 0.0;
            vn2[c_j] = 0.0;
