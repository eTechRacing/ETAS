...
typedef enum {
    SS_DATAFLOW_TOKEN_CONSTRAINT_UNSPECIFIED = 0, /* Default - no block implementation */
    SS_DATAFLOW_TOKEN_CONSTRAINT_NONE,            /* No constraint */
    SS_DATAFLOW_TOKEN_CONSTRAINT_MULTIPLE,        /* Token size must be a multiple of a specified value */
    SS_DATAFLOW_TOKEN_CONSTRAINT_EXPLICIT,        /* Token size must be an explicitly specified value */
    SS_DATAFLOW_TOKEN_CONSTRAINT_IMMUTABLE,       /* Port dims[0] cannot be cascaded - block is not vectorizable */
    SS_DATAFLOW_TOKEN_CONSTRAINT_POWER2           /* Token size must be a power of 2 */
} SS_DataflowTokenConstraintType;

/* 
 * Input port dataflow token constraint info
 */
struct _ssInPortTokenConstraint {
    SS_DataflowTokenConstraintType type;
    uint_T size;
};

/* 
 * Output port dataflow token constraint info
 */
struct _ssOutPortTokenConstraint {
    SS_DataflowTokenConstraintType type;
    uint_T size;
};
...
struct _ssPortInfo2 {
    ...
    struct _ssInPortTokenConstraint    *inputTokenConstraint;
    struct _ssOutPortTokenConstraint   *outputTokenConstraint;
    void *reserved[4];
};

struct _ssSTInfo2 {
    size_t*    vdrInstanceIndex;
...
typedef enum {
    ...
    GEN_FCN_SET_DISALLOW_VARIABLE_DISCRETE,
    GEN_FCN_SET_VARIABLE_DISCRETE_SAMPLE_TIME,
    GEN_FCN_SET_VARIABLE_DISCRETE_SAMPLE_TIME_UID,
    GEN_FCN_SET_INPUT_VARIABLE_DISCRETE_SAMPLE_TIME,
    GEN_FCN_SET_INPUT_VARIABLE_DISCRETE_SAMPLE_TIME_UID,
    GEN_FCN_SET_OUTPUT_VARIABLE_DISCRETE_SAMPLE_TIME,    
    GEN_FCN_SET_OUTPUT_VARIABLE_DISCRETE_SAMPLE_TIME_UID,
...
# define ssSetInputPortUnit(S,port,val) \
 ...
 /* InputTokenConstraint - For each input port of your S-function block, this is the
  *   dataflow token constraint. If unspecified, then the default type is 
  * "SS_DATAFLOW_TOKEN_CONSTRAINT_UNSPECIFIED" and the default size is 1.
  */
# define ssGetInputPortTokenType(S,port) \
    (S)->blkInfo.blkInfo2->portInfo2->inputTokenConstraint[(port)].type
# define ssGetInputPortTokenSize(S,port) \
    (S)->blkInfo.blkInfo2->portInfo2->inputTokenConstraint[(port)].size
# define ssSetInputPortTokenConstraint(S,port,t,sz)                 \
    (S)->blkInfo.blkInfo2->portInfo2->inputTokenConstraint[(port)].type = (t); \
    (S)->blkInfo.blkInfo2->portInfo2->inputTokenConstraint[(port)].size = (sz)
...
# define ssSetOutputPortUnit(S,port,val) \
 ...
 /* OutputTokenConstraint - For each output port of your S-function block, this is the
  *   dataflow token constraint. If unspecified, then the default type is 
  * "SS_DATAFLOW_TOKEN_CONSTRAINT_UNSPECIFIED" and the default size is 1.
  */
# define ssGetOutputPortTokenType(S,port)                               \
    (S)->blkInfo.blkInfo2->portInfo2->outputTokenConstraint[(port)].type
# define ssGetOutputPortTokenSize(S,port)                               \
    (S)->blkInfo.blkInfo2->portInfo2->outputTokenConstraint[(port)].size
# define ssSetOutputPortTokenConstraint(S,port,t,sz)                 \
    (S)->blkInfo.blkInfo2->portInfo2->outputTokenConstraint[(port)].type = (t); \
    (S)->blkInfo.blkInfo2->portInfo2->outputTokenConstraint[(port)].size = (sz)
...
#define _ssSetPortInfo2ForInputTokenConstraint(S, ptr) \
    (S)->blkInfo.blkInfo2->portInfo2->inputTokenConstraint = (ptr)

#define _ssGetPortInfo2ForInputTokenConstraint(S) \
    (S)->blkInfo.blkInfo2->portInfo2->inputTokenConstraint

#define _ssSetPortInfo2ForOutputTokenConstraint(S, ptr) \
    (S)->blkInfo.blkInfo2->portInfo2->outputTokenConstraint = (ptr)

#define _ssGetPortInfo2ForOutputTokenConstraint(S) \
    (S)->blkInfo.blkInfo2->portInfo2->outputTokenConstraint
...
/*
 * Simulink Function API
 *
 * This API is for registering, accessing Simulink Functions
 */
 ...
 */
# define ssQuerySimulinkFunction(S, _scopedFcnName, _queryType) \
    ({\
    _ssFunctionQueryInfo      locQueryInfo; \
    locQueryInfo.scopedFcnName = _scopedFcnName; \
    locQueryInfo.queryType     = _queryType; \
    _ssSafelyCallGenericFcnStart(S)((S),GEN_FCN_QUERY_SIMULINK_FUNCTION, 0, &locQueryInfo)\
    _ssSafelyCallGenericFcnEnd;\
    locQueryInfo.queryResult; \
    })

# define ssDeclareSimulinkFunction(S, _fcnPrototype, _fcnPtr, _fcnVisibility) \
    {\
    _ssFunctionAttributeInfo locAttrInfo;  \
    locAttrInfo.fcnPrototype  = _fcnPrototype; \
    locAttrInfo.fcnVisibility = _fcnVisibility; \
    locAttrInfo.fcnPtr        = _fcnPtr;        \
    _ssSafelyCallGenericFcnStart(S)((S),GEN_FCN_DECLARE_SIMULINK_FUNCTION, 0, &locAttrInfo)\
    _ssSafelyCallGenericFcnEnd;\
    }

# define ssDeclareFunctionCaller(S, _fcnPrototype) \
    {\
    _ssSafelyCallGenericFcnStart(S)((S),GEN_FCN_DECLARE_SIMULINK_FUNCTION_CALLER, 0, _fcnPrototype)\
    _ssSafelyCallGenericFcnEnd;\
    }

# define ssCallSimulinkFunction(S, _scopedFcnName, _numInArgs, _execInArgs, _numOutArgs, _execOutArgs) \
    {\
     _ssFunctionExecArguments  locExecArgs; \
     locExecArgs.scopedFcnName = _scopedFcnName; \
     locExecArgs.numInArgs     = _numInArgs; \
     locExecArgs.inArgs        = _execInArgs; \
     locExecArgs.numOutArgs    = _numOutArgs; \
     locExecArgs.outArgs       = _execOutArgs; \
    _ssSafelyCallGenericFcnStart(S)((S),GEN_FCN_CALL_SIMULINK_FUNCTION, 0, &locExecArgs)\
    _ssSafelyCallGenericFcnEnd;\
    }

# define ssGetSimulinkFunctionInput(_execArgs, _inArgIdx) \
    ((_execArgs)->inArgs[(_inArgIdx)].dataPtr)
    
# define ssGetSimulinkFunctionOutput(_execArgs, _outArgIdx) \
    ((_execArgs)->outArgs[(_outArgIdx)].dataPtr)

# define ssSetSimulinkFunctionArgComplexity(S, _scopedFcnName, _argName, val) \
    {\
    _ssFunctionArgAttributeInfo locAttrInfo; \
    locAttrInfo.scopedFcnName = _scopedFcnName; \
    locAttrInfo.argName       = _argName; \
...
# define ssSetSimulinkFunctionArgDataType(S, _scopedFcnName, _argName, val) \
    {\
    _ssFunctionArgAttributeInfo locAttrInfo; \
    locAttrInfo.scopedFcnName = _scopedFcnName; \
    locAttrInfo.argName       = _argName; \
...
# define ssSetSimulinkFunctionArgDimensions(S, _scopedFcnName, _argName, _numDims, val) \
    {\
    _ssFunctionArgAttributeInfo locAttrInfo; \
    locAttrInfo.scopedFcnName = _scopedFcnName; \
    locAttrInfo.argName       = _argName; \
    locAttrInfo.argNumDims    = _numDims; \
...
#define ssSetDisallowVariableDiscrete(S, flag)                                        \
{     boolean_T tmp = flag;                                                               \
      _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_SET_DISALLOW_VARIABLE_DISCRETE, 0, &tmp) \
      _ssSafelyCallGenericFcnEnd;                                                         \
}

#define ssSetVariableDiscreteSampleTime(S, sti, base)                                                \
{     real_T tmp = base;                                                                             \
      _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_SET_VARIABLE_DISCRETE_SAMPLE_TIME, (int) sti, &tmp) \
      _ssSafelyCallGenericFcnEnd;                                                                    \
}

#define ssSetVariableDiscreteSampleTimeUID(S, sti, uid)                                               \
    _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_SET_VARIABLE_DISCRETE_SAMPLE_TIME_UID, (int) sti, uid) \
    _ssSafelyCallGenericFcnEnd;                                                                       \
...
#if defined(RTW_GENERATED_S_FUNCTION)
#define ssSetInputPortVariableDiscreteSampleTime(S, portIdx, base)                                             \
{     real_T tmp = base;                                                                                       \
      _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_SET_INPUT_VARIABLE_DISCRETE_SAMPLE_TIME, (int) portIdx, &tmp) \
      _ssSafelyCallGenericFcnEnd;                                                                              \
}

#define ssSetInputPortVariableDiscreteSampleTimeUID(S, portIdx, uid)                                            \
    _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_SET_INPUT_VARIABLE_DISCRETE_SAMPLE_TIME_UID, (int) portIdx, uid) \
    _ssSafelyCallGenericFcnEnd


#define ssSetOutputPortVariableDiscreteSampleTime(S, portIdx, base)                                             \
{     real_T tmp = base;                                                                                        \
      _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_SET_OUTPUT_VARIABLE_DISCRETE_SAMPLE_TIME, (int) portIdx, &tmp) \
      _ssSafelyCallGenericFcnEnd;                                                                               \
}


#define ssSetOutputPortVariableDiscreteSampleTimeUID(S, portIdx, uid)                                            \
    _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_SET_OUTPUT_VARIABLE_DISCRETE_SAMPLE_TIME_UID, (int) portIdx, uid) \
      _ssSafelyCallGenericFcnEnd
...
#define ssSetRuntimeThreadSafetyCompliance(S, v) \
    (S)->blkInfo.sfcnFlags.runtimeThreadSafetyCompliance = (v)
