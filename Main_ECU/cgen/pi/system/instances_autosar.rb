...
# c:/program files (x86)/etas/intecrio4.7/build/common/tools/pi_core-4.6/templates/instances_autosar.tpl
# DO NOT EDIT!
...
output(file, "    self->#{node.node_table[:coder]}Wrapper_#{node.node_table[:cname]}_Ptr = initInstance_#{node.node_table[:type]}Wrapper((uint32)&#{node.node_table[:modpath]}, sizeof(#{node.node_table[:cdt]}), (uint32)#{node.node_table[:kind]});")
end
if(node.node_table[:kind]=="ASD_VARIABLE")
output(file, "    self->#{node.node_table[:coder]}Wrapper_#{node.node_table[:cname]}_Ptr = initInstance_#{node.node_table[:type]}Wrapper((uint32)&#{node.node_table[:modname]}_proxy_#{node.node_table[:id]}, sizeof(#{node.node_table[:cdt]}), (uint32)#{node.node_table[:kind]});")
end
end
    end
end
